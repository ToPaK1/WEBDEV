...
function ensureDb() {
  ...
  if (!fs.existsSync(DB_FILE)) fs.writeFileSync(DB_FILE, JSON.stringify({ users: [], messages: [], projects: [] }, null, 2));
}
function readDb() {
  ensureDb();
  try { return JSON.parse(fs.readFileSync(DB_FILE, 'utf8')); }
  catch { return { users: [], messages: [], projects: [] }; }
}
function writeDb(db) {
  ensureDb();
...
app.post('/api/auth/signup', rateLimit({ max: 8 }), (req, res) => {
  ...
  res.status(201).json({ token: createToken({ sub: user.id, ...safeUser }), user: safeUser });
});
...
app.post('/api/auth/login', rateLimit({ max: 10 }), (req, res) => {
  ...
  if (email === ADMIN_EMAIL && password === ADMIN_PASSWORD) {
    ...
    return res.json({ token: createToken({ sub: admin.id, ...admin }), user: admin });
  }
  ...
  const safeUser = { id: user.id, name: user.name, email: user.email, role: user.role };
  res.json({ token: createToken({ sub: user.id, ...safeUser }), user: safeUser });
});
...
app.post('/api/contact', rateLimit({ max: 6 }), (req, res) => {
  ...
  if (name.length < 2 || name.length > 80 || !isValidEmail(email) || business.length > 80 || message.length < 10 || message.length > 5000) {
    return res.status(400).json({ message: 'Please provide a valid name, email, business type and project details (10-5000 characters).' });
  }
  const db = readDb();
  const item = { id: crypto.randomUUID(), name, email, business, message, status: 'new', createdAt: new Date().toISOString() };
...
app.get('/api/admin/dashboard', auth, adminOnly, (_req, res) => {
  const db = readDb();
  res.json({ stats: { customers: db.users.length, messages: db.messages.length, unreadMessages: db.messages.filter(item => item.status === 'new').length }, customers: db.users.map(({ id, name, email, role, createdAt }) => ({ id, name, email, role, createdAt })), messages: db.messages });
});
...
app.listen(PORT, () => {
  ...
  console.log('Contact messages are stored in data/webdev.json.');
});
